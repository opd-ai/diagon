...
- Stage 3: Diagon runs its full first-party readonly unit-test suite (`GOFLAGS=-mod=readonly go test ./...`); pinned third-party components (Store, Paywall) are verified for reproducible dependency-lock state by compiling every package under `-mod=readonly` in the pinned toolchain (their own repositories own executing their unit tests, which include non-hermetic network/crypto tests). Lockfile cleanliness is enforced after each component runs
