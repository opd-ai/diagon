...
- [x] 2026-07-02: Added an OpenBSD variant alongside Debian that reuses the shared profile, validation policy, service contracts, bootstrap profile, and `diagonctl` tooling, but runs a separate CI pipeline (`.github/workflows/openbsd.yml`). It emits a directly bootable/installable raw disk image via OpenBSD `autoinstall(8)` under QEMU (`.github/scripts/build-openbsd-image.sh`), driven by a shared OpenBSD config set (`profiles/openbsd/install.conf` autoinstall response, `install.site` package hook, `pkg.list`) that mirrors the Debian preseed/package intent, plus a dedicated build matrix (`.github/openbsd-matrix.json`) and guard tests. Owner: Diagon
